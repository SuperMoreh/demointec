...
export class ReportsToolsService {
  ...
  async generateToolsReport(tools: ToolsCatalog[], title: string = 'CATÁLOGO DE HERRAMIENTAS'): Promise<void> {
    ...
    const header = (data: any) => {
      ...
      if (isFirstPage) {
        const marginRight = 20; // igual que el margin.right de autoTable
        const dateBoxWidth = 60;
        const dateBoxHeight = 6;
        const dateBoxX = pageWidth - marginRight - dateBoxWidth + 4;
        const dateBoxY = 24 + 4; // 15px ≈ 4mm debajo de la línea
        // Recuadro azul para el label
    ...
    const tableBody = tools.map(tool => [
      this.truncateText(tool.name_tool, 28),
      this.truncateText(tool.code, 12),
      this.truncateText(tool.description, 32),
    ...
    const fileName = `catalogo_herramientas_${new Date().toISOString().split('T')[0]}.pdf`;
    pdf.save(fileName);
  }

  async generateToolsReportFromHTML(tools: ToolsCatalog[], title: string = 'Catálogo de Herramientas'): Promise<void> {
    // Usar el método programático en lugar del HTML para mejor control de páginas
    await this.generateToolsReport(tools, 'CATÁLOGO DE HERRAMIENTAS');
  }
  ...
  private createReportHTML(tools: ToolsCatalog[], title: string): HTMLElement {
