...
export class ReportSubcategoriesService {
  ...
  async generateSubcategoriesReport(subcategories: Subcategory[], categories: Category[], title: string = 'CATÁLOGO DE SUBPARTIDAS'): Promise<void> {
    ...
    const tableBody = subcategories.map(sub => [
    ...
    autoTable(pdf, {
      startY: 50,
      margin: { top: 44, left: 10, right: 10, bottom: 30 },
      head: [['Nombre', 'Característica 1', 'Característica 2', 'Partida', 'Estado']],
      body: tableBody,
      theme: 'grid',
      headStyles: {
    ...
    const fileName = `catalogo_subpartidas_${new Date().toISOString().split('T')[0]}.pdf`;
    pdf.save(fileName);
  }
} 