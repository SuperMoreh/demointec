...
export class ReportProjectsService {
  ...
  async generateProjectsReport(projects: Project[], title: string = 'CATÁLOGO DE OBRAS'): Promise<void> {
    ...
    const tableBody = projects.map(proj => [
      ...
      proj.locality,
      proj.official,
      proj.status ? 'Activo' : 'Inactivo'
    ]);
    autoTable(pdf, {
      startY: 50,
      margin: { top: 44, left: 20, right: 20, bottom: 30 },
      head: [['Nombre', 'Tipo de Ubicación', 'Localidad', 'Oficial', 'Estado']],
      body: tableBody,
      theme: 'grid',
      headStyles: {
    ...
    const fileName = `catalogo_obras_${new Date().toISOString().split('T')[0]}.pdf`;
    pdf.save(fileName);
  }
} 