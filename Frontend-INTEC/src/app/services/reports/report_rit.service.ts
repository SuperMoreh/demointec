...
export class ReportRitService {
  ...
  private drawDocument(doc: jsPDF, data: RitData): void {
    ...
    y = this.drawArticle(doc, 'ARTICULO 10.- ', 'Cuando se trabaja el día de descanso, las horas normales de jornada NO se contabilizan como tiempo extra, sino como descanso laborado. Si por necesidades se excede la jornada, el tiempo de más, si se contabiliza como tiempo extra. (Si una persona trabaja en su descanso, un turno según el artículo III, su pago seria, sino hubo ausencia en la semana, de 6 + 1 días, mas 2 días por descanso laborado, pero si labora un turno más 2hrs. extras, su pago sería, ni no hubo faltas, de 6 + 1 días, más 2 días por descanso laborado, más 2hrs. extras)', y); y += 2;
    ...
    y = this.drawItem(doc, 'B.   Para efectos de pago en la nómina se computará el tiempo normal y extraordinario de Lunes a Domingo.', 8, y); y += 3;
    ...
    y = this.drawArticle(doc, 'ARTICULO 12.\u2013 ', 'La justificación y comprobación de la asistencia y puntualidad de los trabajadores y empleados a sus labores se harán a través de una lista de asistencia, donde el trabajador registrará sus horarios de entrada y de salida por escrito. Dicho registro deberá de ser personalmente por el trabajador o empleado al iniciar y terminar cada turno de trabajo, teniendo estrictamente prohibido hacerlo otra persona por él.', y); y += 2;
    ...
    y = this.drawItem(doc, '3. FALTAS CON AVISO (PERMISO). - Sujeto al Art. V.', 4, y);
    ...
    y = this.drawArticle(doc, 'ARTICULO 31.- ', 'En caso que el trabajador su pago se efectué quincenalmente este se llevarán a cabo los días 15 y ultimo de cada mes, en caso de que el pago de salario al trabajador será por semana cubriéndose el sábado de cada una de ellas.', y); y += 2;
    ...
    const obl51 = [
      ...
      'XX.  Cuidar, mantener en bues estado las sillas proporcionadas para su descanso durante la jornada.',
