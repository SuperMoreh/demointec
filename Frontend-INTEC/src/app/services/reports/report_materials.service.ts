...
export class ReportMaterialsService {
  ...
  async generateMaterialsReport(materials: MaterialsCatalog[], title: string = 'CATÁLOGO DE MATERIALES'): Promise<void> {
    ...
    const tableBody = materials.map(mat => [
      ...
      mat.subcategory,
      mat.unit,
      mat.status ? 'Activo' : 'Inactivo'
    ]);
    autoTable(pdf, {
      startY: 50,
      margin: { top: 44, left: 6, right: 6, bottom: 30 },
      head: [['Nombre', 'Código', 'C1', 'C2', 'Categoría', 'Subcategoría', 'Unidad', 'Estado']],
      body: tableBody,
      theme: 'grid',
      headStyles: {
    ...
    const fileName = `catalogo_materiales_${new Date().toISOString().split('T')[0]}.pdf`;
    pdf.save(fileName);
  }
} 