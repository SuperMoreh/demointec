...
export class ReportEmployeeProjectService {
  ...
  async generateEmployeeProjectReport(data: EmployeeProject[], title: string = 'ASIGNACIÓN DE OBRAS'): Promise<void> {
    ...
    autoTable(pdf, {
      ...
      head: [['Nombre', 'Correo', 'Obra', 'Estatus']],
      body: tableBody,
      theme: 'grid',
      headStyles: {
    ...
    const fileName = `asignacion_obras_${new Date().toISOString().split('T')[0]}.pdf`;
    pdf.save(fileName);
  }
}
