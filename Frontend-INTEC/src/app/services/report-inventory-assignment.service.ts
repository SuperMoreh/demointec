...
export class ReportInventoryAssignmentService {

  exportToExcel(data: InventoryAssignment[], getInventoryName: (id: string) => string, getEmployeeName: (id: string) => string): void {
    const colCount = 8;
    const rows: any[][] = [];

    rows.push(['REPORTE DE ASIGNACIONES DE INVENTARIO']);
    rows.push([`Fecha de emisión: ${new Date().toLocaleString('es-ES', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}`]);
    rows.push([]);
    rows.push(['Artículo', 'Colaborador', 'Cantidad', 'Fecha Asignación', 'Fecha Devolución', 'Estado', 'Responsable', 'Observaciones']);
    ...
    ws['!cols'] = [
      { wch: 30 }, { wch: 30 }, { wch: 10 }, { wch: 16 },
      { wch: 16 }, { wch: 14 }, { wch: 20 }, { wch: 30 }
    ];
    ...
    XLSX.writeFile(wb, `Asignaciones_Inventario_${new Date().toISOString().slice(0, 10)}.xlsx`);
  }
}
